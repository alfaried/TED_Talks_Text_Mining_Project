...
class LDA_Mallet:
    ...
    def get_pretrained_model(self, model_name="train_model_48"):
    ...
    def retrieve_related_transcript_id(self, topic_id):
        if topic_id == None:
            raise Exception("topic_id required")

        transcript_id_list = []

        for transcript_name in os.listdir(self.clustered_data_folder + str(topic_id)):
            transcript_id = transcript_name.split(".")[0].split("_")[1]
